...
// Updated by Rasmus Andersson to include only a subset of the "GFM" additions.
...
Showdown.converter = function() {
  ...
  // RA: if not false, id attributes are added for all headers
  this.anchorPrefix = "";
  ...
  // Added by RA: converts text to a valid dom name (id or class name)
  var domnameReservedRE = /[^a-zA-Z0-9_\.-]+/g;
  function mkDOMName(title){
    return title.toLowerCase().replace(domnameReservedRE, '-')
      .replace(/\-+/g, '-');
  }
  function hhblock(level, title) {
    title = _RunSpanGamut(title);
    var html = "<h" + level;
    if (typeof self.anchorPrefix === 'string') {
      var id = self.anchorPrefix + mkDOMName(title);
      html += ' id="'+id+'">';
      if (self.anchorLinks) {
  ...
  var _DoHeaders = function(text) {
    ...
    */
    ...
    text = text.replace(/^(\#{1,6})[ \t]*(.+?)[ \t]*\#*\n+/gm,
      function(wholeMatch,m1,m2) { return hhblock(m1.length, m2); });
